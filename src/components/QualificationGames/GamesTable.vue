  ...
  <v-layout>
    <v-flex class="elevation-1">
      <v-toolbar dark flat color="primary">
        <v-toolbar-title>نتایج بازی‌های انتخابی</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-btn flat icon right :loading="gamesUpdateLoading" @click="handleGamesUpdate">
          <v-icon>cached</v-icon>
        </v-btn>
      </v-toolbar>
      <v-data-table :headers="headers" :items="games" :pagination.sync="pagination" hide-actions>
        <template slot="items" slot-scope="props">
          <td class="text-xs-center">{{ props.item.team1_name }}</td>
          <td
            class="text-xs-center"
          >{{ props.item.team1_goals === null ? '-' : props.item.team1_goals }}</td>
          <td
            class="text-xs-center"
          >{{ props.item.team2_goals === null ? '-' : props.item.team2_goals }}</td>
          ...
          <td class="text-xs-center">{{ props.item.run_date | faDate }}</td>
          <td class="text-xs-center">
            <v-btn
...
export default {
  ...
  computed: mapState({
    games: state => state.teamInfo.games.filter(game => game.type === "انتخابی")
  }),
  methods: {
    handleGamesUpdate() {
